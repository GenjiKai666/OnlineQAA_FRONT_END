<template>
  ...
  <div>
    <p>当前选择作业答案序号:{{ certainAssignmentCommittedID }}</p>
    <p>当前用户ID:{{ id }}</p>
    <p>当前是否老师:{{ teacher }}</p>
  ...
  <div style="padding: 10px 0">
    ...
    <el-input
        ...
        :suffix-icon="ElIconSearch"
        placeholder="请输入题干"
        v-model="certainQuestion"
    ></el-input>
    <el-input
        style="width: 500px"
        class="ml-5"
        :suffix-icon="ElIconSearch"
        placeholder="请输入截止时间,格式参考时间戳"
        v-model="certainDeadline"
    ></el-input>
    <el-input
        style="width: 500px"
        class="ml-5"
        :suffix-icon="ElIconSearch"
        placeholder="请输入是否为小组作业,1为是,0为否"
        v-model="certainGroup"
    ></el-input>
    <el-button type="primary" @click="publish">布置</el-button>
  </div>
  <div>
    <el-table
        :data="tableData"
...
export default {
  ...
  data() {
    return {
      ...
      certainQuestion:null,
      certainDeadline:null,
      certainGroup:null,
      form: {},
      //默认对话弹框不要展示
      ...
      this.request
          .get(
              '/assignment/publishAssignment',
              {
                params: {
                  teacherid:this.id,
                  question:this.certainQuestion,
                  deadline:this.certainDeadline,
                  group:this.certainGroup,
                },
              }
          )
    }
